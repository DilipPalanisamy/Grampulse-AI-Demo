...
const getPriorityBadgeConfig = (tier = 'P3') => {
  ...
  switch (normTier) {
    case 'P1':
      return {
        badgeClass: 'bg-rose-500/15 text-rose-600 dark:text-rose-400 border-rose-500/40 ring-1 ring-rose-500/20',
    ...
    case 'P2':
      return {
        badgeClass: 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/40 ring-1 ring-amber-500/20',
    ...
    default:
      return {
        badgeClass: 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/40 ring-1 ring-emerald-500/20',
...
const SchemeRecommendations = ({ schemes = [], loading = false }) => {
  ...
  if (loading) {
    return (
      <div className="bg-[var(--bg-card)] backdrop-blur-md rounded-2xl border border-[var(--border-subtle)] p-6 space-y-4 animate-pulse">
        <div className="flex items-center justify-between pb-3 border-b border-[var(--border-subtle)]">
          <div className="h-6 w-48 bg-[var(--bg-primary)] rounded" />
          <div className="h-6 w-24 bg-[var(--bg-primary)] rounded-full" />
        </div>
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-36 bg-[var(--bg-primary)] rounded-xl border border-[var(--border-subtle)] p-4 space-y-2">
              <div className="h-4 w-3/4 bg-[var(--border-subtle)] rounded" />
              <div className="h-3 w-1/2 bg-[var(--border-subtle)] rounded" />
              <div className="h-12 w-full bg-[var(--bg-card)] rounded-lg" />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="bg-[var(--bg-card)] backdrop-blur-md rounded-2xl border border-[var(--border-subtle)] p-5 sm:p-6 shadow-2xl space-y-5">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[var(--border-subtle)] pb-4">
        ...
        <span className="text-xs font-mono text-[var(--text-main)] font-bold bg-[var(--bg-primary)] px-3 py-1 rounded-lg border border-[var(--border-subtle)] self-start sm:self-center">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-0.5">
          ...
          {p1Count > 0 && (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                selectedFilter === 'P1'
                  ? 'bg-rose-500/25 text-rose-600 dark:text-rose-300 border border-rose-500/50 shadow-sm'
                  : 'bg-[var(--bg-primary)] text-rose-600 dark:text-rose-400 hover:text-rose-500 border border-rose-500/20'
              }`}
            >
          ...
          {p2Count > 0 && (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                selectedFilter === 'P2'
                  ? 'bg-amber-500/25 text-amber-600 dark:text-amber-300 border border-amber-500/50 shadow-sm'
                  : 'bg-[var(--bg-primary)] text-amber-600 dark:text-amber-400 hover:text-amber-500 border border-amber-500/20'
              }`}
            >
          ...
          {p3Count > 0 && (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                selectedFilter === 'P3'
                  ? 'bg-emerald-500/25 text-emerald-600 dark:text-emerald-300 border border-emerald-500/50 shadow-sm'
                  : 'bg-[var(--bg-primary)] text-emerald-600 dark:text-emerald-400 hover:text-emerald-500 border border-emerald-500/20'
              }`}
            >
      ...
      <div className="space-y-4">
        {filteredSchemes.length === 0 ? (
          <div className="text-center py-10 text-[var(--text-muted)] text-xs bg-[var(--bg-primary)] rounded-2xl border border-[var(--border-subtle)] p-6 space-y-2">
            <Info className="w-7 h-7 mx-auto text-slate-500" />
            <p className="font-bold text-[var(--text-main)] text-sm">No schemes found in this category.</p>
          ...
          filteredSchemes.map((scheme, idx) => {
            ...
            return (
              <div
                ...
                className={`group relative overflow-hidden bg-[var(--bg-primary)] rounded-2xl border border-[var(--border-subtle)] ${badgeConfig.borderHover} p-4 sm:p-5 space-y-3.5 transition-all duration-300 shadow-md hover:shadow-xl`}
              >
                {/* 1. Header: Scheme Index, Name, Ministry & Badges */}
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <span className="w-7 h-7 rounded-xl bg-[var(--bg-card)] text-[var(--text-main)] font-mono text-xs font-black flex items-center justify-center shrink-0 mt-0.5 border border-[var(--border-subtle)] shadow-inner">
                      {idx + 1}
                    </span>

                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="text-sm sm:text-base font-extrabold text-[var(--text-main)] group-hover:text-emerald-500 transition-colors">
                          {scheme.scheme_name}
                        </h3>
                        ...
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-[var(--bg-card)] text-[var(--text-muted)] border border-[var(--border-subtle)]">
                          {scheme.category}
                        </span>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 bg-[var(--bg-card)] p-3.5 rounded-xl border border-[var(--border-subtle)] text-xs">
                  ...
                  <div className="sm:col-span-7 space-y-1">
                    ...
                    <p className="text-[var(--text-main)] text-[11px] leading-snug">
                  ...
                  <div className="sm:col-span-5 flex flex-col justify-center sm:items-end border-t sm:border-t-0 sm:border-l border-[var(--border-subtle)] pt-2.5 sm:pt-0 sm:pl-3.5">
                    ...
                    {scheme.budget_formula_breakdown && (
                      <span className="text-[9.5px] text-[var(--text-muted)] font-mono text-right mt-0.5">
                        {scheme.budget_formula_breakdown}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-2.5 border-t border-[var(--border-subtle)]">
                  <span className="text-[10.5px] text-[var(--text-subtle)] font-mono flex items-center gap-1">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold text-[var(--text-main)] bg-[var(--bg-card)] hover:bg-[var(--bg-card-hover)] border border-[var(--border-subtle)] hover:border-emerald-500/50 transition-all shadow-sm cursor-pointer active:scale-95 group/btn"
                  >
                    <span>View Official Guidelines &amp; Apply</span>
                    <ArrowUpRight className="w-3.5 h-3.5 text-emerald-500 group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-transform" />
                  </a>
                </div>
              </div>
            );
